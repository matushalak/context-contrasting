...
from context_contrasting.minimal.config_s import minimal_configs3 as minimal_configs_s
from context_contrasting.minimal.minimal_s import CCNeuron
from context_contrasting.minimal.utils import (
    build_res,
    collect_outputs,
    prepare_collect,
    _rename_phase,
...
from context_contrasting.minimal.visualize_s import (
...
def run_experiment(
    ...
    training_X_long, training_C_long = _build_training_stimuli(n_steps_per_phase=2 * n_steps_per_phase, n_trials=10)
    occluded_training_X = torch.zeros_like(training_X_long)

    naive_frames: list[DataFrame] = []
    expert_frames: list[DataFrame] = []
    expert2_frames: list[DataFrame] = []
    ...
    occluded_training_frame = run_experimental_phase(
        model,
        occluded_training_X,
        training_C_long,
    ...
    df = pd_concat(
        [
            ...
            *(frame.assign(experiment_series=OCCLUDED_ONLY_EXPERIMENT_SERIES) for frame in expert_frames),
            occluded_training_frame.assign(experiment_series=OCCLUDED_ONLY_EXPERIMENT_SERIES),
            *(frame.assign(experiment_series=OCCLUDED_ONLY_EXPERIMENT_SERIES) for frame in expert2_frames),
        ],
        ignore_index=True,
    )
