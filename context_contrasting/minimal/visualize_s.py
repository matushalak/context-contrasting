...
def _summarize_windowed_repeated_zscored_trace(
    long_df: DataFrame,
    *,
    condition: str,
    phase: str,
    ...
    zscored = (stacked - baseline_mean) / scale
    y_mean = zscored.mean(axis=0)
    if zscored.shape[0] > 1:
        y_sem = zscored.std(axis=0, ddof=1) / np.sqrt(zscored.shape[0])
    else:
        ...
        for phase_index, phase in enumerate(phases):
            ...
            for condition_index, condition in enumerate(selected_conditions):
                ...
                for trace_index, trace_type in enumerate(trace_types):
                    summary = _summarize_windowed_repeated_zscored_trace(
                        long_df,
                        condition=condition,
                        phase=phase,
...
def _plot_panel_a_activity(
    ...
    *,
    ...
    for idx, (condition, phase) in enumerate(activity_layout):
        ...
        if idx == 0:
            ax.set_ylabel("Z-scored activity", fontsize=AXIS_LABEL_FONTSIZE)
            ax.legend(
...
def save_grouped_transition_panels(
    ...
    for transition_name in transition_order:
        ...
        combined_transitions[transition_name] = pd.concat(merged_parts, ignore_index=True)
...
def visualize_novel_condition_quickplot(
    long_df: DataFrame,
    save_path: str = PLOTSDIR,
    name: str | None = None,
    ...
    phases = _resolve_phase_sequence(long_df)
    pre_post_df = long_df.loc[long_df["experiment_phase"].isin(phases)].copy()
    novel_conditions = _resolve_condition_sequence(
        [condition for condition in pre_post_df["condition"].dropna().unique().tolist() if _is_novel_condition(str(condition))]
    )
    if not novel_conditions:
        return

    novel_df = pre_post_df.loc[pre_post_df["condition"].isin(novel_conditions)].copy()
    ...
    activity_layout = [(condition, phase) for condition in novel_conditions for phase in phases]
    fig, axes = plt.subplots(
        1,
        max(1, len(activity_layout)),
        figsize=(6 * max(1, len(activity_layout)), 5.5),
        squeeze=False,
