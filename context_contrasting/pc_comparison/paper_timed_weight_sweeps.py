...
from context_contrasting.pc_comparison.pc_templates import FULL_PROTOCOL_CALIBRATED_LEARNING_RATE
...
PC_SWEEPS: tuple[PCSweepSpec, ...] = (
    PCSweepSpec("pyc_pv", "pyc", "pv", "lat"),
    PCSweepSpec("pyc_lat", "pyc", "lat", "pv"),
    PCSweepSpec("pv_lat", "pv", "lat", "pyc"),
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--learning-rate", type=float, default=FULL_PROTOCOL_CALIBRATED_LEARNING_RATE)
...
def _make_pc_model(
    ...
    pyc_profile: TuningProfile,
    pv_profile: TuningProfile,
    *,
    pyc_value: float,
    pv_value: float,
    lat_value: float,
    ...
    return CorrectPCneuron(
        ...
        n_context=N_FEATURES,
        circuit=circuit,
        activation=ThresholdReLU(threshold=0.0, subtractive=False, hasMax=True, maxValue=1.0),
        learning_rate=float(args.learning_rate),
        pyc_excitatory_init=_init_spec(
            _profile_values(pyc_value, pyc_profile, float(args.untuned_weight))
        ),
        pv_excitatory_init=_init_spec(
            _profile_values(pv_value, pv_profile, float(args.untuned_weight))
        ),
...
def _pc_values(spec: PCSweepSpec, x_value: float, y_value: float, fixed_value: float) -> dict[str, float]:
    values = {"pyc": fixed_value, "pv": fixed_value, "lat": fixed_value}
    values[spec.x_parameter] = x_value
    values[spec.y_parameter] = y_value
    return values


def _simulate_pc_pair(
    pyc_profile: TuningProfile,
    pv_profile: TuningProfile,
    *,
    spec: PCSweepSpec,
    ...
    test_stimuli, training_stimuli = _paper_stimuli(meta)
    values = _pc_values(spec, x_value, y_value, float(args.fixed_weight))
    responses: dict[str, tuple[dict[str, dict[str, float]], dict[str, dict[str, float]]]] = {}
    ...
    for circuit in ("PPE", "NPE"):
        model = _make_pc_model(
            ...
            pyc_profile,
            pv_profile,
            pyc_value=values["pyc"],
            pv_value=values["pv"],
            lat_value=values["lat"],
            args=args,
            meta=meta,
        )
        pre, baseline = _probe_model(model, meta, test_stimuli)
        run_experimental_phase(
            ...
            training_stimuli[0],
            training_stimuli[1],
            "full_familiar_training",
            update=True,
        )
        post = _probe_model_with_baseline(model, meta, test_stimuli, baseline)
        responses[circuit] = (pre, post)
        row: dict[str, Any] = {
            "model": circuit,
            "profile": f"pyc_{pyc_profile.name}__pv_{pv_profile.name}",
            "profile_label": f"PyC {pyc_profile.label}; PV {pv_profile.label}",
            "sweep": spec.name,
            "x_init": float(x_value),
            "y_init": float(y_value),
            "fixed_init": float(args.fixed_weight),
    ...
    combined_pre = _sum_response_dicts(responses["PPE"][0], responses["NPE"][0])
    combined_post = _sum_response_dicts(responses["PPE"][1], responses["NPE"][1])
    combined = dict(rows[0])
    combined["model"] = "PPE+NPE"
    _fill_response_columns(combined, combined_pre, combined_post)
    _transition_flags(combined, float(meta["sector_threshold"]))
...
def _run_pc(args: argparse.Namespace, meta: dict[str, Any]) -> pd.DataFrame:
    grid = np.linspace(float(args.grid_min), float(args.grid_max), int(args.grid_size))
    tasks = [
        (pyc_profile, pv_profile, spec, float(x), float(y))
        for pyc_profile in PROFILES
        for pv_profile in PROFILES
        for spec in PC_SWEEPS
        for x in grid
        for y in grid
    ]
    nested_rows = Parallel(n_jobs=int(args.n_jobs))(
        delayed(_simulate_pc_pair)(
            pyc_profile,
            pv_profile,
            spec=spec,
            x_value=x,
        ...
        for pyc_profile, pv_profile, spec, x, y in tasks
    )
    return pd.DataFrame([row for task_rows in nested_rows for row in task_rows])
...
def _write_outputs(df: pd.DataFrame, args: argparse.Namespace, meta: dict[str, Any], label: str) -> None:
    ...
    for (model, profile, sweep), rows in df.groupby(["model", "profile", "sweep"], observed=True):
        if model == "CC":
            ...
            spec = next(spec for spec in PC_SWEEPS if spec.name == sweep)
            parameter_labels = {
                "pyc": "initial PyC excitation",
                "pv": "initial PV excitation",
                "lat": "initial w_LAT",
            }
            x_label = parameter_labels[spec.x_parameter]
            y_label = parameter_labels[spec.y_parameter]
            stem = f"{profile}_{model.lower().replace('+', '_')}_{sweep}"
        _plot_heatmap(
            rows,
        ...
        json.dumps(
            {
                ...
                "pc_sweeps": [spec.__dict__ for spec in PC_SWEEPS],
                "pc_parameter_sharing": "Each grid point uses the same PyC, PV, and w_LAT parameters for PPE and NPE.",
            },
            indent=2,
        )
        + "\n"
