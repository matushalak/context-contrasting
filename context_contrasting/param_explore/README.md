...
- `python -m context_contrasting.param_explore.run_sbi_expert_states ...`
- `python -m context_contrasting.param_explore.analyze_basins --results-dir ...`
- `python -m context_contrasting.param_explore.analyze_expert_states --results-dir ...`
...
- optional `expert_state_subclasses/` subdirectory with endpoint-conditioned
  plots, counts, and parameter-range summaries
- optional wide-box SBI result directories with endpoint-conditioned posterior
  samples, accepted-region summaries, and expert-response component planes
...
Expert-state conditioned analysis:

- expert endpoints are defined independently as `FF`, `FB`, `un`, and `FF&FB`
- naive FF subclasses are computed from familiar vs novel full-response activity
- naive FB subclasses are computed from familiar vs novel occluded-response activity
- plots are generated separately for familiar and novel expert states, then
  separately for each endpoint and naive subclass family
