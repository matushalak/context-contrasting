...
figure('Position', [45 439 1600 471]);
sz = 45;

% Pre
axPreColor = subplot(1,3,1);
scatter(axPreColor, scatFullPreSort, scatOcclPreSort, sz, colors, 'filled');
hold(axPreColor, 'on')
xlim(axPreColor, [-1 3]); ylim(axPreColor, [-1 3]);
xticks(axPreColor, -1:1:3); yticks(axPreColor, -1:1:3);
draw_diag45(axPreColor);
xlabel(axPreColor, 'NO fam pre'); ylabel(axPreColor, 'O fam pre');
title(axPreColor, 'Pre — colored by pre response blend');
axis(axPreColor, 'square')

% Task
axTaskColor = subplot(1,3,2);
scatter(axTaskColor, scatFullTaskSort, scatOcclTaskSort, sz, colors, 'filled');  % same colors
hold(axTaskColor, 'on')
xlim(axTaskColor, [-1 3]); ylim(axTaskColor, [-1 3]);
xticks(axTaskColor, -1:1:3); yticks(axTaskColor, -1:1:3);
draw_diag45(axTaskColor);
xlabel(axTaskColor, 'NO fam task'); ylabel(axTaskColor, 'O fam task');
title(axTaskColor, 'Task — colored by pre response blend');
axis(axTaskColor, 'square')

% Displacement vectors: each neuron starts at its pre point and points to its task point
axDispColor = subplot(1,3,3);
hold(axDispColor, 'on')
dx = scatFullTaskSort - scatFullPreSort;
dy = scatOcclTaskSort - scatOcclPreSort;
scatter(axDispColor, scatFullPreSort, scatOcclPreSort, sz * 0.35, colors, ...
    'filled', 'MarkerFaceAlpha', 0.35);
for iNeuron = 1:numel(scatFullPreSort)
    quiver(axDispColor, scatFullPreSort(iNeuron), scatOcclPreSort(iNeuron), dx(iNeuron), dy(iNeuron), ...
        0, 'Color', colors(iNeuron,:), 'LineWidth', 1, 'MaxHeadSize', 0.8);
end
xlim(axDispColor, [-1 3]); ylim(axDispColor, [-1 3]);
xticks(axDispColor, -1:1:3); yticks(axDispColor, -1:1:3);
draw_diag45(axDispColor);
xlabel(axDispColor, 'NO fam'); ylabel(axDispColor, 'O fam');
title(axDispColor, 'Pre → Task displacement');
axis(axDispColor, 'square')
...
% ---- ALL NEURONS: colored scatter with selection on top + leader lines (numbers) on PRE, TASK & DISPLACEMENT ----
figure('Position', [45 439 1600 471]);
sz = 45;
...
draw_diag45(ax1);
xlabel(ax1,'NO fam pre'); ylabel(ax1,'O fam pre'); title(ax1,'Pre — colored by pre response blend');
axis(ax1, 'square')
...
draw_diag45(ax2);
xlabel(ax2,'NO fam task'); ylabel(ax2,'O fam task'); title(ax2,'Task — colored by pre response blend');
axis(ax2, 'square')
...
% ---------- Displacement vectors (ALL) ----------
axDispAll = subplot(1,3,3);
hold(axDispAll, 'on')
dxAllPlot = scatFullTaskPlot - scatFullPrePlot;
dyAllPlot = scatOcclTaskPlot - scatOcclPrePlot;
scatter(axDispAll, scatFullPrePlot, scatOcclPrePlot, sz * 0.35, colorsPlot, ...
    'filled', 'MarkerFaceAlpha', 0.35);
for iNeuron = 1:numel(scatFullPrePlot)
    quiver(axDispAll, scatFullPrePlot(iNeuron), scatOcclPrePlot(iNeuron), dxAllPlot(iNeuron), dyAllPlot(iNeuron), ...
        0, 'Color', colorsPlot(iNeuron,:), 'LineWidth', 1, 'MaxHeadSize', 0.8);
end
xlim(axDispAll, [-1 3]); ylim(axDispAll, [-1 3]); xticks(axDispAll, -1:1:3); yticks(axDispAll, -1:1:3);
draw_diag45(axDispAll);
xlabel(axDispAll,'NO fam'); ylabel(axDispAll,'O fam'); title(axDispAll,'Pre → Task displacement');
axis(axDispAll, 'square')
draw_leader_labels(axDispAll, preX_sel, preY_sel, selNums_inOrder);
...
figure('Position', [45 100 1600 471]);
sz = 55;
...
title(ax4,'Selection only — task');
axis(ax4, 'square')
draw_leader_labels(ax4, taskX_only, taskY_only, selNumsLab);

% ---------- Displacement vectors (SELECTION ONLY) ----------
ax5 = subplot(1,3,3);
hold(ax5, 'on')
dxSel = taskX_only - preX_only;
dySel = taskY_only - preY_only;
scatter(ax5, preX_only, preY_only, sz * 0.35, colorsSel, ...
    'filled', 'MarkerFaceAlpha', 0.35);
for iNeuron = 1:numel(preX_only)
    quiver(ax5, preX_only(iNeuron), preY_only(iNeuron), dxSel(iNeuron), dySel(iNeuron), ...
        0, 'Color', colorsSel(iNeuron,:), 'LineWidth', 1.2, 'MaxHeadSize', 0.8);
end
xlim(ax5, [-1 3]); ylim(ax5, [-1 3]); xticks(ax5, -1:1:3); yticks(ax5, -1:1:3);
draw_diag45(ax5);
xlabel(ax5,'NO fam'); ylabel(ax5,'O fam');
title(ax5,'Selection only — displacement');
axis(ax5, 'square')
draw_leader_labels(ax5, preX_only, preY_only, selNumsLab);
