...
figure('Position', [20 360 1900 620]);
sz = 45;
angleClusterK = 3;

% ---------- Pre ----------
axPreColored = subplot(1,4,1);
...
figure('Position', [80 40 1500 500]);
axAngleCircleColored = subplot(1,3,1);
draw_displacement_angle_circle(axAngleCircleColored, scatFullPreSort, scatOcclPreSort, ...
    scatFullTaskSort, scatOcclTaskSort, colors, sz, 'Angles — colored by pre response blend');

axAngleCircleClustered = subplot(1,3,2);
draw_displacement_angle_circle_clusters(axAngleCircleClustered, scatFullPreSort, scatOcclPreSort, ...
    scatFullTaskSort, scatOcclTaskSort, angleClusterK, sz, 'Angles — colored by angle cluster');

axAngleMeanVectors = subplot(1,3,3);
draw_angle_cluster_mean_vectors(axAngleMeanVectors, scatFullPreSort, scatOcclPreSort, ...
    scatFullTaskSort, scatOcclTaskSort, angleClusterK, sz, 'Angle clusters — mean displacement');
...
% ---------- Separate angle diagnostics figure (ALL) ----------
figure('Position', [80 40 1500 500]);
ax2d = subplot(1,3,1);
...
ax2e = subplot(1,3,2);
draw_displacement_angle_circle_clusters(ax2e, scatFullPreSort, scatOcclPreSort, ...
    scatFullTaskSort, scatOcclTaskSort, angleClusterK, sz, 'Angles — colored by angle cluster');

ax2f = subplot(1,3,3);
draw_angle_cluster_mean_vectors(ax2f, scatFullPreSort, scatOcclPreSort, ...
    scatFullTaskSort, scatOcclTaskSort, angleClusterK, sz, 'Angle clusters — mean displacement');
...
figure('Position', [20 80 1900 620]);
sz = 55;
angleClusterK = 3;
...
% ---------- Separate angle diagnostics figure (SELECTION ONLY) ----------
figure('Position', [80 40 1500 500]);
ax7 = subplot(1,3,1);
draw_displacement_angle_circle(ax7, preX_only, preY_only, taskX_only, taskY_only, ...
    colorsSel, sz, 'Selection only — angles by pre color');

ax8 = subplot(1,3,2);
draw_displacement_angle_circle_clusters(ax8, preX_only, preY_only, taskX_only, taskY_only, ...
    angleClusterK, sz, 'Selection only — angles by cluster');

ax9 = subplot(1,3,3);
draw_angle_cluster_mean_vectors(ax9, preX_only, preY_only, taskX_only, taskY_only, ...
    angleClusterK, sz, 'Selection only — angle cluster mean vectors');
...
function draw_displacement_angle_circle_clusters(ax, preX, preY, taskX, taskY, k, sz, plotTitle)
    % Plot displacement-vector angles on the unit circle, colored by angular k-means cluster.
    axes(ax); hold(ax, 'on');
    preX = preX(:); preY = preY(:); taskX = taskX(:); taskY = taskY(:);
    dx = taskX - preX;
    dy = taskY - preY;
    ...
    th = linspace(-pi, pi, 361);
    plot(ax, cos(th), sin(th), 'k-', 'LineWidth', 0.5, 'HandleVisibility', 'off');
    plot(ax, [-1.1 1.1], [0 0], 'k:', 'LineWidth', 0.5, 'HandleVisibility', 'off');
    plot(ax, [0 0], [-1.1 1.1], 'k:', 'LineWidth', 0.5, 'HandleVisibility', 'off');

    if sum(valid) < k
        text(ax, 0, 0, sprintf('Need at least %d non-zero displacement vectors', k), ...
            'HorizontalAlignment', 'center');
    else
        theta = atan2(dy, dx);
        legHandles = gobjects(k, 1);
        legLabels = strings(k, 1);
        for ci = 1:k
            ixC = valid & angleClusterIdx == ci;
            legHandles(ci) = scatter(ax, cos(theta(ixC)), sin(theta(ixC)), sz * 0.75, clusterColors(ci,:), ...
                'filled', 'MarkerFaceAlpha', 0.8, 'MarkerEdgeColor', 'k', 'LineWidth', 0.25);
            legLabels(ci) = sprintf('A%d, n = %d', ci, sum(ixC));
        end
        legend(ax, legHandles, cellstr(legLabels), 'Location', 'bestoutside', 'Box', 'off');
    end
...
function draw_angle_cluster_mean_vectors(ax, preX, preY, taskX, taskY, k, sz, plotTitle)
    % Cluster neurons by displacement-vector angle and plot the mean displacement vector per cluster.
    % This diagnostic is in displacement space: arrows start at (0,0), not at the mean Pre location.
    axes(ax); hold(ax, 'on');
    preX = preX(:); preY = preY(:); taskX = taskX(:); taskY = taskY(:);
    dx = taskX - preX;
    dy = taskY - preY;
    ...
    % Individual displacement endpoints as faint context.
    for ci = 1:k
        ixC = valid & angleClusterIdx == ci;
        scatter(ax, dx(ixC), dy(ixC), sz * 0.25, clusterColors(ci,:), 'filled', ...
            'MarkerFaceAlpha', 0.18, 'MarkerEdgeColor', 'none', 'HandleVisibility', 'off');
    end

    plot(ax, [0 0], [-10 10], 'k:', 'LineWidth', 0.5, 'HandleVisibility', 'off');
    plot(ax, [-10 10], [0 0], 'k:', 'LineWidth', 0.5, 'HandleVisibility', 'off');

    legHandles = gobjects(k, 1);
    legLabels = strings(k, 1);
    maxAbsVal = 0;

    % Mean displacement vector for each angular cluster, all drawn from the origin.
    for ci = 1:k
        ixC = valid & angleClusterIdx == ci;
        if ~any(ixC)
            continue
        ...
        maxAbsVal = max(maxAbsVal, max(abs([dx(ixC); dy(ixC); meanDx; meanDy])));

        draw_vector_arrow(ax, 0, 0, meanDx, meanDy, clusterColors(ci,:), 0.75, 0.22, 0.08);
        legHandles(ci) = plot(ax, nan, nan, '-', 'Color', clusterColors(ci,:), 'LineWidth', 2);
        legLabels(ci) = sprintf('A%d, n = %d', ci, sum(ixC));
    end

    if maxAbsVal == 0 || ~isfinite(maxAbsVal)
        maxAbsVal = 1;
    end
    lim = max(0.25, 1.15 * maxAbsVal);
    xlim(ax, [-lim lim]); ylim(ax, [-lim lim]);
    xlabel(ax, '\Delta NO fam'); ylabel(ax, '\Delta O fam');
    title(ax, plotTitle);
    axis(ax, 'square');
    legend(ax, legHandles, cellstr(legLabels), 'Location', 'bestoutside', 'Box', 'off');
end
