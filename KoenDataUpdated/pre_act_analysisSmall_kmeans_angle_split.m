...
function draw_displacement_angle_circle_clusters(ax, preX, preY, taskX, taskY, k, sz, plotTitle)
    ...
    if sum(valid) < k
        ...
        for ci = 1:k
            ...
            scatter(ax, cos(theta(ixC)), sin(theta(ixC)), sz * 0.75, clusterColors(ci,:), ...
...
function draw_angle_cluster_mean_vectors(ax, preX, preY, taskX, taskY, k, sz, plotTitle)
    % Cluster neurons by displacement-vector angle and plot the mean Pre -> Task vector per cluster.
    axes(ax); hold(ax, 'on');
    preX = preX(:); preY = preY(:); taskX = taskX(:); taskY = taskY(:);
    dx = taskX - preX;
    dy = taskY - preY;
    ...
    % Show Task points colored by angular cluster as context.
    for ci = 1:k
        ixC = valid & angleClusterIdx == ci;
        scatter(ax, taskX(ixC), taskY(ixC), sz * 0.45, clusterColors(ci,:), 'filled', ...
    ...
    for ci = 1:k
        ...
        scatter(ax, meanPreX, meanPreY, sz * 1.15, clusterColors(ci,:), 'filled', ...
